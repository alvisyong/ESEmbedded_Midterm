...
//	???????? //use HSE for PLL source
  SET_BIT(RCC_BASE+RCC_PLLCFGR_OFFSET, PLLSRC_BIT) ;
	...
	//????????
	SET_BIT(RCC_BASE+RCC_CR_OFFSET,PLLRDY_BIT);
...
//	????????
	...
	//????????
  SET_BIT(RCC_BASE+FLASH_ACR_OFFSET,PRFTEN_BIT 8);
	//set flash wait state = 5
//	????????
  WRITE_BITS(RCC_BASE + FLASH_ACR_OFFSET, LATENCY_2_BIT 2, LATENCY_0_BIT 0);
	//use pll
//	????????
  SET_BIT(RCC_BASE+RCC_CFGR_OFFSET,SWS_1_BIT);
  CLEAR_BIT(RCC_BASE+RCC_CFGR_OFFSET,SWS_0_BIT);
	//wait
	while (~(((READ_BIT(RCC_BASE + RCC_CFGR_OFFSET, SW_1_BIT))==0)&&((READ_BIT(RCC_BASE + RCC_CFGR_OFFSET, SW_1_BIT))==0)))
		;
